...
const combineHurufAngka = (letra, angka) => {
    return letra.reduce((arr, h) => {
        angka.forEach(a => {
            arr.push(h + a);
        })
        return arr;
...
// Mata o adversário
const eat = (removeSquare, posicaoAtual) => {
    delete posicaoAtual[removeSquare];
    return posicaoAtual;
}
...
const getKingMoves = (square, pieces, currBoard) => {
    const [letra, angka] = [...square];
    let j = parseInt(angka) - 1;
    let k = parseInt(angka) + 1;
    let newSquare = [];

    getMovesKingLeftRight(i => i >= "a", getNextChar(letra, -1), j, k, pieces, currBoard, -1)
        .forEach(m => newSquare.push(m));
    getMovesKingLeftRight(i => i <= "h", getNextChar(letra, 1), j, k, pieces, currBoard, 1)
        .forEach(m => newSquare.push(m));
    return newSquare
        .reduce((arr, m) => {
            arr.push({
...
// Mover Peão
const obterMovimentosPeao = (square, pieces, currBoard) => {
    const [letra, angka] = [...square];
    const newAngka = [];
    const newHuruf = [];
    ...
    if (angka < 8 && (warna == "w" || (warna == "b" && backMove))) newAngka.push(parseInt(angka) +
        1);
    if (letra > 'a') newHuruf.push(getNextChar(letra, -1));
    if (letra < 'h') newHuruf.push(getNextChar(letra, 1));
...
const getMovesEat = (currSquare, nextSquare, pieces, currBoard) => {
    ...
    if (newA) {
        let newSquare;
        if (h > letra && h < "h") newSquare = getNextChar(h, 1) + newA;
        else if (h < letra && h > "a") newSquare = getNextChar(h, -1) + newA;
...
const getMoves = (square, pieces = null, currBoard = null, recur = false) => {
    ...
    if (pieces[1].toLowerCase() == "p") moves = obterMovimentosPeao(square, pieces, currBoard);
